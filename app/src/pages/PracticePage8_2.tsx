    ...
    const handleSubmit = useCallback(async () => {
        ...
        switch (current.type) {
            case 'mc': {
                ...
                studentAnswer = current.options[selectedOption];
            ...
            case 'multiInput': {
                ...
                isCorrect = current.fields.every((f) => {
                    const val = normalize(multiValues[f.key] ?? '');
                    const expected = normalize(current.answers[f.key] ?? '');
                    if (val === expected) return true;
                    // Check accept alternatives
                    const alts = current.accept?.[f.key];
                    if (alts) return alts.some((a) => normalize(val) === normalize(a));
                    return false;
                });
                break;
            }
